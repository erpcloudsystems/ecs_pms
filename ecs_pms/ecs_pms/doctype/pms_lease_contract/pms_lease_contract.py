...
from frappe.utils import (flt, getdate, get_url, now, nowtime, get_time, today, get_datetime, add_days)
...
class PMSLeaseContract(Document):
    ...
    def validate(self):
        ...
        self.end_date = add_days(end_date, days=(- 1))
        ...
        if self.revenue_type == "Fixed Lease":
            self.calculate_repayment_schedule_fixed()
        if self.revenue_type == "Revenue Share + Fixed Rent":
            self.calculate_repayment_schedule_fixed_rent()
        if self.revenue_type == "Revenue Share / Minimum Rent":
            self.calculate_repayment_schedule_minimum_rent()
        if self.revenue_type == "Revenue Share":
            self.calculate_repayment_schedule_revenue_share()
    ...
    def contract_defaults(self):
        ...
        mode_of_payment_account = frappe.db.get_value('Mode of Payment Account', {'parent': mode_of_payment},
                                                      'default_account')
    ...
    @frappe.whitelist()
    def calculate_repayment_schedule_fixed(self):
        monthly_marketing = 0
        daily_marketing = 0
    ...
    @frappe.whitelist()
    def calculate_repayment_schedule_minimum_rent(self):
        monthly_marketing = 0
        daily_marketing = 0
        monthly_maintenance = 0
        daily_maintenance = 0
        ...
        x = self.minimum_rent + fixed_value
        x1 = x + (x * self.annual_increase / 100)
        x2 = x1 + (x1 * self.annual_increase / 100)
        x3 = x2 + (x2 * self.annual_increase / 100)
        x4 = x3 + (x3 * self.annual_increase / 100)
        ...
        for d in self.contract_repayment_schedule:
            ...
            if d.minimum_rent < d.revenue_share:
                d.monthly_payment = d.revenue_share
            d.total = d.monthly_payment + d.marketing + d.maintenance
            d.base_total = (d.monthly_payment + d.marketing + d.maintenance) * self.conversion_rate
            d.net_total = d.monthly_payment + d.marketing + d.maintenance
            d.base_net_total = (d.monthly_payment + d.marketing + d.maintenance) * self.conversion_rate
    ...
    @frappe.whitelist()
    def calculate_repayment_schedule_fixed_rent(self):
        monthly_marketing = 0
        daily_marketing = 0
        monthly_maintenance = 0
        daily_maintenance = 0
        ...
        fixed_rent = self.fixed_rent1
        fixed_value = self.fixed_rent1 * self.annual_increase / 100
        a = self.total_no_of_months - self.grace_period
        b = 1

        x = self.fixed_rent1 + fixed_value
        x1 = x + (x * self.annual_increase / 100)
        x2 = x1 + (x1 * self.annual_increase / 100)
        x3 = x2 + (x2 * self.annual_increase / 100)
        x4 = x3 + (x3 * self.annual_increase / 100)
        ...
        while (a > 0):
            self.append("contract_repayment_schedule", {
                ...
                "base_fixed_rent": fixed_rent * self.conversion_rate,
                "marketing": monthly_marketing,
                "base_marketing": monthly_marketing * self.conversion_rate,
                "maintenance": monthly_maintenance,
                "base_maintenance": monthly_maintenance * self.conversion_rate,
            ...
            if b >= (12 - self.grace_period) and b <= (23 - self.grace_period):
                ...
                maintenance_value = s
                daily_maintenance = maintenance_value / 30
                fixed_rent = rent_value
                monthly_marketing = marketing_value
                monthly_maintenance = maintenance_value

            if b >= (24 - self.grace_period) and b <= (35 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (2 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (36 - self.grace_period) and b <= (47 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (3 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (48 - self.grace_period) and b <= (59 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (4 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (60 - self.grace_period) and b <= (71 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (5 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (72 - self.grace_period) and b <= (83 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (6 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (84 - self.grace_period) and b <= (95 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (7 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (96 - self.grace_period) and b <= (107 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (8 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (108 - self.grace_period) and b <= (119 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (9 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (120 - self.grace_period) and b <= (131 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (10 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (132 - self.grace_period) and b <= (143 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (11 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (144 - self.grace_period) and b <= (155 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (12 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (156 - self.grace_period) and b <= (167 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (13 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (168 - self.grace_period) and b <= (179 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (14 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (180 - self.grace_period) and b <= (191 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (15 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (192 - self.grace_period) and b <= (203 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (16 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (204 - self.grace_period) and b <= (215 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (17 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (216 - self.grace_period) and b <= (227 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (18 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (228 - self.grace_period) and b <= (239 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (19 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
            ...
            if b >= (240 - self.grace_period) and b <= (251 - self.grace_period):
                if self.annual_increase_type == "Fixed Rate":
                    rent_value = self.fixed_rent1 + (20 * fixed_value)
                    fixed_rent = rent_value
                    daily_rent_value = rent_value / 30
        ...
        if self.no_of_days > 0:
            ...
            self.append("contract_repayment_schedule", {
                "payment_date": payment_date,
                "fixed_rent": daily_rent_value * self.no_of_days,
                "base_fixed_rent": (daily_rent_value * self.no_of_days) * self.conversion_rate,
                "marketing": daily_marketing * self.no_of_days,
                "base_marketing": (daily_marketing * self.no_of_days) * self.conversion_rate,
                "maintenance": daily_maintenance * self.no_of_days,
                "base_maintenance": (daily_maintenance * self.no_of_days) * self.conversion_rate,
        ...
        total = 0
        for d in self.contract_repayment_schedule:
            d.monthly_payment = d.fixed_rent
            d.total = d.monthly_payment + d.marketing + d.maintenance
            d.base_total = (d.monthly_payment + d.marketing + d.maintenance) * self.conversion_rate
            d.net_total = d.monthly_payment + d.marketing + d.maintenance
            d.base_net_total = (d.monthly_payment + d.marketing + d.maintenance) * self.conversion_rate
    ...
    @frappe.whitelist()
    def calculate_repayment_schedule_revenue_share(self):
        monthly_marketing = 0
        monthly_maintenance = 0
        fixed_value1 = 0
        fixed_value2 = 0
        ...
        total = 0
        for d in self.contract_repayment_schedule:
            d.revenue_share = d.revenue_share_total * d.sales_percentage
            d.monthly_payment =  d.revenue_share
            d.total = d.monthly_payment + d.marketing + d.maintenance
            d.base_total = (d.monthly_payment + d.marketing + d.maintenance) * self.conversion_rate
            d.net_total = d.monthly_payment + d.marketing + d.maintenance
    ...
    def calculate_totals(self):
        ...
        # self.base_meter_price = (self.rent_value_ / (self.internal_space + self.external_space)) * self.conversion_rate
    ...
    def insurance_payment(self):
        ...
        self.reload()
        frappe.msgprint(
            " Insurance Payment " + "<a href=/app/payment-entry/" + new_doc.name + ">" + new_doc.name + "</a>" + " Created Successfully ")
    ...
    def advanced_payment(self):
        ...
        frappe.msgprint(
            " Advanced Payment " + "<a href=/app/payment-entry/" + new_doc.name + ">" + new_doc.name + "</a>" + " Created Successfully ")
    ...
    def journal_cancel(doc, method=None):
        if doc.reference_doctype == "Lease Contract" and doc.bill_no:
            frappe.db.sql(
                """update `tabJournal Entry` set reference_link ='' where bill_no='{bill_no}'""".format(
                    bill_no=bill_no))
    ...
    @frappe.whitelist()
    def promotion_rent_invoice(self):
        new_doc = frappe.get_doc({
        ...
        items.item_code = self.lease_item
        items.rate = self.promotion_rent_amount
        items.item_tax_template = self.default_tax_template
        items.cost_center = self.cost_center
        items.income_account = self.income_account
        items.qty = 1
        items.conversion_factor = 1
        items.uom = "Nos"
        items.description = "القيمة الايجارية عن عقد رقم  " + "(" + self.name + ") "
        taxes = new_doc.append("taxes", {})
        taxes.charge_type = "On Net Total"
        taxes.account_head = "2-03-01-0012 - ضريبة الجدول 1% - DMM"
        taxes.rate = 1
        taxes.description = "2-03-01-0012 - ضريبة الجدول 1% - DMM"
        new_doc.insert(ignore_permissions=True)
        frappe.msgprint(
            "Sales Invoice" + "<a href=/app/sales-invoice/" + new_doc.name + ">" + new_doc.name + "</a>" + " Created Successfully ")

    @frappe.whitelist()
    def create_promotion__payment_entry(self):
        default_mode_of_payment = frappe.db.get_single_value("Contract Defaults", "default_mode_of_payment")
        mode_of_payment_account = frappe.db.get_value('Mode of Payment Account', {'parent': default_mode_of_payment},
                                                      'default_account')
        ...
        new_doc = frappe.get_doc({
            ...
            "party_type": "Customer",
            "party": self.party,
            "paid_amount": self.promotion_outstanding_amount,
            "received_amount": self.promotion_outstanding_amount,
            "source_exchange_rate": 1,
            "target_exchange_rate": 1,
            "paid_to": mode_of_payment_account
        })
